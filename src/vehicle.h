...
public:
	...
	Vehicle(GameState* g, Texture* t, Point2D<int> p, Vector2D<float> s) : Crosser(g, t, p, s, VEHICLE_BACKJUMP) {
	}

	//Constructor que carga un vehiculo desde el archivo
	Vehicle(GameState* g, std::istream& input) : Crosser(g, input, VEHICLE1_TEXTURE_NR, VEHICLE_BACKJUMP) {}
